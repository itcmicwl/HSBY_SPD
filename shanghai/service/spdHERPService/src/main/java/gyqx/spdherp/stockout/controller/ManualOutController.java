...
import static gyqx.spdherp.stockout.Constance.OUTSTOCK_KIND_MANUAL;
...
public class ManualOutController extends BaseController {
    ...
    public AjaxResult<String> saveOutStockBill(@Valid @RequestBody OutStockBillVo outStockBillVo, Errors errors) throws Exception {
        filterErrors(errors);
        if((!outStockBillVo.getOutStockKind().equals(OUTSTOCK_KIND_MANUAL.toString()) &&
                !outStockBillVo.getOutStockKind().equals(OUTSTOCK_KIND_CONSUME.toString())) ||
                StringUtils.hasText(outStockBillVo.getInDeptId()) ||
                StringUtils.hasText(outStockBillVo.getInOrgId())){
