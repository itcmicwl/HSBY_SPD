<template>
<!-- 请购扫码入库-->
  <el-row id="in_apply_edit" class="full-height" v-loading="diaLoging">
      ...
      <el-table :data="inStockBill.lstDetail" style="width: 100%;" class="mytable" border v-loading='tloading'>
        ...
        <el-table-column label="退货数量" prop="inQty" width="90">  
          <template slot-scope="scope">{{scope.row.inQty | filterReturn}}</template>                      
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    checkRow4wym(code){
      ...
      this.tloading = true;
      let _this = this;
      this.getOutListByUniqueCode(code).then(res=>{
