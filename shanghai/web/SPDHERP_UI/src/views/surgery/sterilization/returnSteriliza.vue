		...
		<div v-if="editFormVisible==true" style="height: 100%;">
			...
			<template>
				<el-table key="pkgListTable" ref="pkgListTable" :data="surgeryPkgDetail" style="width: 100%;" fit border tooltip-effect="dark" class="pkgListTableHeight"  @selection-change="handleSelectionChange" :row-class-name="tableRowClassName">
					...
					<el-table-column prop="secSterilizationQty" label="数量" width="50">
					</el-table-column>
					<el-table-column label="消毒人" width="120">
							<template slot-scope="scope">
								<el-input placeholder="请输入消毒人" v-model="scope.row.secSterilizer">
								</el-input>
							</template>
					</el-table-column>
					<el-table-column label="消毒时间" width="200">
							<template slot-scope="scope">
								<el-date-picker v-model="scope.row.secSterilizeDate" type="datetime" placeholder="消毒时间" value-format="yyyy-MM-dd hh:mm:ss" >
								</el-date-picker>
							</template>
					</el-table-column>
...
export default {
  ...
  methods: {
    ...
    getSterailizationPkgs: function(pIndex) {
      ...
      this.loading = true;
      this.axios.post("/spdHERPService/surgery/sterilization/getSecSterilizationPkgByPage", param).then(
        res => {
          this.loading = false;
      ...
      this.loading = true;
      this.axios.post("/spdHERPService/surgery/sterilization/getSecSterilizationPkgExpireByPage", param).then(
        res => {
          this.loading = false;
					this.sterilizationPkgList = res.data.data.data;
          this.total = res.data.data.total;
    ...
    getSurPkgListDetail(surCode, pIndex) {
      ...
      this.axios
        .post("/spdHERPService/surgery/sterilization/getSecSterilizationListByPage", param)
        .then(
          res => {
            if (res.data.code == "0") {
							...
							this.surgeryPkgDetail.forEach(item=>{
								item.secSterilizationQty = item.qty - item.sumUseQty
								if(!item.secSterilizer){
									item.sterilizeExpdate = null
									item.secSterilizeDate = null
								}
							})
            }
          },
    ...
    submitSterilization: function() {
        ...
        return
			...
			this.multipleSelection.some(item=>{
				if(!item.secSterilizer || !item.secSterilizeDate || !item.sterilizeExpdate){
					checkFlag = false;
				}
			})
			if(!checkFlag){
      ...
      let arr = new Array();
			this.multipleSelection.forEach((item, index)=>{
				...
				const sterilizaDate = moment(item.secSterilizeDate);
      ...
      this.axios
        .post("/spdHERPService/surgery/sterilization/updateSecSterilization", this.sterilizaListBill)
        .then(
          res => {
            this.loading = false;
            if (res.data.code == "0") {
