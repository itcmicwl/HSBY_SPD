<template>
    <div id="car"  style="height:95%">
        <el-row  height="auto"  style="height:60%">
            <el-col   style="padding-left: 5px;height:100%">
                ...
                <el-table highlight-current-row  class="carbuy-tableheight"
                         :data="hlGoodsList" style="width: 100%"
                ...
                <el-pagination @size-change="handleGoodsSizeChange" @current-change="handleGoodsCurrentChange" :current-page="hlGoodsPage" :page-sizes="[10, 20, 50, 100]" :page-size="hlGoodsPageSize" layout="total, sizes, prev, pager, next" :total="hlGoodsTotal" style="float:right;" small>
			          </el-pagination>        
            </el-col>
        </el-row>

        <el-row  height="auto"    style="height:40%;padding-top:5px">     
            <el-col :span="6" class="toolbar" style="padding-bottom: 0px; padding-left:5px; height: 98%;">
                <el-table highlight-current-row class="carbuySur-tableheight"
                         :data="ssPkgList" style="width: 100%"
                          ref="multipleTable4Sur"
                          row-key='surId'  @selection-change="handleSelectionChange4Sur"
                          ...
                          :default-sort="{prop: 'surId', order: 'descending'}">
                            ...
                            <el-table-column label="请购包数"  width="100">
                                            <template slot-scope="scope">
                                                <el-input-number placeholder="请输入数量"   v-model="scope.row.needSurPkgQty" size="mini" :min="0" class="col-input-num"  :step="1"   @change="qtySurPacketChange($event,scope.row)">
                                                </el-input-number>
                                            </template>
                           </el-table-column>
                ...
                <el-pagination @size-change="handleSsPkgSizeChange" @current-change="handleSsPkgCurrentChange" :current-page="ssPkgPage" :page-sizes="[10, 20, 50, 100]" :page-size="ssPkgPageSize" layout="total, sizes, prev, pager, next" :total="ssPkgTotal" style="float:right;" small>
			          </el-pagination>
            </el-col>
            <el-col :span="18" class="toolbar" style="padding-bottom: 0px; padding-left:5px; height: 98%;">
                <el-table highlight-current-row class="carbuySurGoods-tableheight"
                         :data="ssGoodsList" style="width: 100%"
                          ...
                          :default-sort="{prop: 'goodsId', order: 'descending'}">
                            ...
                            <el-table-column prop="goodsName"   min-idth="85" label="商品名称" width="150" header-align="center" show-overflow-tooltip >
                             ...
                             <el-table-column prop="qty"   min-idth="85" label="每包数量" width="150" header-align="center" show-overflow-tooltip >
                            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getDeptCarGoodsStVo: function (pIndex, cmd) {
      ...
      var param = { orderBy: '',
        queryObject: {hosId: this.user.corpId,
          ...
          buyKind: 0},
        page: this.hlGoodsPage,
        rows: this.hlGoodsPageSize
      }
      //调用非手术包数据
     this.axios.post('/spdHERPService/applyMgr/deptBuyCar/getDeptCarGoodsStVo', param).then(res => {
        this.loading = false
        if (res.data.code == '0') {
          this.hlGoodsList = res.data.data.data
          this.hlGoodsList.forEach(g => {
            g.needPacketQty = g.isPacket == 1 ? g.needQty / g.packetCode : 0
          })
          this.hlGoodsTotal = res.data.data.total
   ...
   getSurData(pIndex, cmd){
    //调用手术包数据
     var param = { orderBy: '',
        queryObject: {hosId: this.user.corpId,
          ...
          buyKind: 0},
        page: this.ssPkgPage,
        rows: this.ssPkgPageSize
     }
        this.axios.post('/spdHERPService/applyMgr/deptBuyCar/getDeptCarGoodsStVo4sur', param).then(res => {
            this.loading = false
            if (res.data.code == '0') {
              this.ssPkgList = res.data.data.data       
              this.ssPkgTotal = res.data.data.total           
              if (cmd === 'del' || cmd === 'create') {
                this.multipleSelection4Sur = []
                this_.$refs.multipleTable4Sur.clearSelection()
          }
            } else {
              this.$message.error(res.data.msg)
            }
    ...
    fetchSurGoods(row){
      this.ssGoodsList=row.surDefGoodsLst     
    },
    searchGoods () {
      this.getMixQuery(1, 'query')
    },
    handleGoodsSizeChange (val) {
      this.hlGoodsPageSize = val
      this.getDeptCarGoodsStVo(this.hlGoodsPage, 'query')
    },
    handleGoodsCurrentChange (val) {
      this.hlGoodsPage = val
      this.getDeptCarGoodsStVo(this.hlGoodsPage, 'query')
    },
    ...
    handleSelectionChange4Sur(val){
      ...
      handleSsPkgSizeChange (val) {
      this.ssPkgPageSize = val
      this.getDeptCarGoodsStVo(this.ssPkgPage, 'query')
    },
    handleSsPkgCurrentChange (val) {
      this.ssPkgPage = val
      this.getDeptCarGoodsStVo(this.ssPkgPage, 'query')
    },
    createBill: function () {
    ...
    this.multipleSelection4Sur.forEach(o=>{
                  ...
                  delete o.needQty
                  delete o.surDefGoodsLst
                  o.hosId=this.user.corpId
                  o.deptId= this.user.orgId
                
                  o.buyKind=20
                  o.purMode =40
                //needSurPkgQty
                if(!this.multipleSelection4Main.find(m=>m.surId=== o.surId))
      ...
      this.axios.post('/spdHERPService/applyMgr/deptBuyMain/create4mixSur/' + this.user.userId, this.buyBill).then(res => {
        this.loading = true
        if (res.data.code == '0') {
          this.$message({ message: '操作成功！', type: 'success'})
          this.loading = false
          this.getDeptCarGoodsStVo(1, 'create')
           this.getSurData(1,'create')
        } else {
          this.$message.error(res.data.msg)
				 	}
      }, err => {
    ...
    delCar: function () {
      ...
      this.axios.post('/spdHERPService/applyMgr/deptBuyMain/delCar4mixSur/', this.multipleSelection4Main).then(res => {
        this.loading = true
        if (res.data.code == '0') {
          this.$message({ message: '操作成功！', type: 'success'})
          this.loading = false
    ...
    qtySurPacketChange(val,row){
                  row.hosId=this.user.corpId
                  row.surDefGoodsLst.forEach(g=>{
                  g.hosId=this.user.corpId
                  g.deptId= this.user.orgId
     })
      this.loading = true
      this.axios.post('/spdHERPService/applyMgr/deptBuyCar/updateSurQty',row).then(res => {
        this.loading = false
        if (res.data.code == '0') {
              this.$message.success('保存成功')
            } else {
...
<style scoped>
    ...
    .carbuySur-tableheight{
       height: calc(100% - 10px);
    }
    .carbuySurGoods-tableheight{
       height: calc(100% );
    }
