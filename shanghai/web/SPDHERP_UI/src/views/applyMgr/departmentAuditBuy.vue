                              ...
                              <el-table-column prop="sourceWarehouseName" label="目标科室" sortable> </el-table-column>
                                ...
                                <el-table-column label="操作" width="150">
                                    <template slot-scope="scope">
                                        ...
                                        <el-button v-if="scope.row.state == 10"  @click="auditBill(scope.row)" type="text">科室审核</el-button>
                            ...
                            <template>
                                    <el-row  height="auto"    style="height:100%;padding-top:0px">     
                                        <el-col :span="6" class="toolbar" style="padding-bottom: 0px; padding-left:5px; height:  calc(100% - 30px);">
                                            <el-table highlight-current-row class="surAduti-tableheight"
...
export default {
  data() {
    return {
      ...
      editForm4surVisible:false, //手术包请购单审核界面
      modelTitle: null,
      buyBill: {
        id: "",
        billId: "",
  ...
  methods: {
    ...
    submitBill: function() {
     ...
     if( this.buyBill.subVoList.length==0){
      ...
      this.axios.post("/spdHERPService/applyMgr/departmentAudit/submitBill",this.buyBill)
        .then(
          res => {
            this.loading = false;
            if (res.data.code == "0") {
...
.surAduti-tableheight {
        height: calc(100% - 100px);
}
 
#department-audit-buy .el-dialog__body{
       height:  calc(100% - 150px);
       
       padding-bottom: 0px;

}
#department-audit-buy .el-dialog--center{
       height:  calc(100% - 0px);
       margin-top: 0px;
}
#department-audit-buy .el-dialog__wrapper{
       height:  calc(90% - 0px);
}
</style>