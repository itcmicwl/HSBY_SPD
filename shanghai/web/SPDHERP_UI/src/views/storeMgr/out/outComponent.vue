...
export default {
  ...
  computed: {
    ...
    selectedGoodsAmount() {
      return this.outStockBill.goodsList
  ...
  methods: {
    handleSearchBtn() {
      if (
        this.outStockKind !== OUTSTOCK_BILL_KIND.MANUAL.value &&
        this.outStockKind !== OUTSTOCK_BILL_KIND.CONSUME.value &&
        (!this.currentStock || !this.currentDeptId)
      ) {
            ...
            if (barcodeUtil.validBarcodeUnique(masterBarcode)) {
              this.stockGoodsList.forEach(item => {
                item.selected = cloneDeep(
                  this.batchNoIdUniqueCodeList[item.goodsId].find(
                    (element, index, array) =>
                      element.uniqueCode === masterBarcode
                  )
                );
                if (item.selected) {
              ...
              pkgGoods.forEach(pkgGoodsItem => {
                let item = this.stockGoodsList.find(
                  (element, index, array) =>
                    element.goodsId === pkgGoodsItem.goodsId
                );
                item.selected = cloneDeep(
                  this.batchNoIdUniqueCodeList[item.goodsId].find(
                    (element, index, array) => {
                      return (
                        ...
                        ((!pkgGoodsItem.expiredDate && !element.expiredDate) ||
                          moment(pkgGoodsItem.expiredDate).format(
                            "YYYY-MM-DD"
                          ) ===
              ...
              barcodeResult.forEach(goodsItem => {
                let item = this.stockGoodsList.find(
                  (element, index, array) =>
                    element.goodsId === goodsItem.goodsId
                );
                item.selected = cloneDeep(
                  this.batchNoIdUniqueCodeList[item.goodsId].find(
                    (element, index, array) => {
                      return (
                        (goodsItem.batchNo === element.batchNo ||
                          (!!goodsItem.batchNo && !!element.batchNo)) &&
                        ((!goodsItem.expiredDate && !element.expiredDate) ||
                          moment(goodsItem.expiredDate).format("YYYY-MM-DD") ===
                            moment(element.expiredDate).format("YYYY-MM-DD")) &&
                        (goodsItem.uniqueCode === element.uniqueCode ||
                          (!goodsItem.uniqueCode && !element.uniqueCode))
                      );
                    }
                  )
    ...
    handlePreviewBtnClick() {
      ...
      if (
        this.outStockKind !== OUTSTOCK_BILL_KIND.MANUAL.value &&
        this.outStockKind !== OUTSTOCK_BILL_KIND.CONSUME.value &&
        this.outStockBill.outDeptId.length === 0
      ) {
        this.handleError("请选择领用部门");
        return;
      }
      if (
        this.outStockBill.goodsList.some(
          o => !o.selected || o.selected.id === ""
        )
      ) {
      ...
      if (
        this.outStockBill.goodsList.some(
          o => o.qty > Math.min(o.avlQty, o.selected.qty, o.undoneQty)
        )
      ) {
    ...
    getStockGoodsList() {
      return this.axios.get(this.baseUrl + "/getStockGoodsList", {
        params: {
    ...
    getUniqueCodeList() {
      let distinctGoodsId = new Set(
        this.stockGoodsList.map(item => item.goodsId)
      );
          ...
          .then(res => {
            if (res.data.code === 0) {
              ...
              for (let goodsId of distinctGoodsId) {
                let goodsList = res.data.data.filter(
                  o => o.goodsId === goodsId
                );
                this.$set(this.batchNoIdUniqueCodeList, goodsId, [
                  ...goodsList
                ]);
    ...
    combineDuplicateGoods() {
      ...
      if (this.outStockBill.goodsList.length > goodsLableSet.size) {
        ...
        for (let goodsLabel of goodsLableSet) {
          let sumQty = this.outStockBill.goodsList
            .filter(item => item.selected.id === goodsLabel)
            .reduce((acc, cur) => acc + cur.qty, 0);
          let goodsInfo = this.outStockBill.goodsList.find(
            item => item.selected.id === goodsLabel
          );
    ...
    addRowToCache(row) {
      let item = this.multipleSelection.find(
        o => o.selected.id === row.selected.id
      );
      if (item) {
        item.qty += row.qty;
    ...
    delCacheRow(row) {
      let index = this.multipleSelection.findIndex(
        item => item.selected.id === row.selected.id
      );
      if (index >= 0) {
